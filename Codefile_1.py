...
DATA_DIR = 'D:/Projects/Data_Quality/'
DQ = pd.read_csv(os.path.join(DATA_DIR,'dimpatient.csv'))

print('-------------------------------------------------------')
DQ = DQ[['PATIENTID', 'FIRSTNAME', 'LASTNAME', 'GENDERCODE', 'DATEOFBIRTH', 'ETHNICITYCODE', 'RACECODE', 'MARITALSTATUS', 'DQ_STATUS']]
print('-------------------------------------------------------')
print(DQ.head())
print('-------------------------------------------------------')
#Gender_freq = Counter(DQ.GENDERCODE)
...
print('-------------------------------------------------------')
DQ.ix[DQ.DQ_STATUS == 'INVALID', 'DQ_STATUS'] = 1
DQ.DQ_STATUS = DQ[['DQ_STATUS']].convert_objects(convert_numeric=True).fillna(0)
print(DQ.DQ_STATUS.value_counts())
